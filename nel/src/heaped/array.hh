...
struct Array {
    ...
    // Vector::into_arr, then need to pass Node into array using private ctor.. needs arr::friend
    // vec Array::from(vec), then need to detach node from vec using private func.. needs vec:friend
    // arr rust: as_ptr(&self) -> *const T rust: as_mut_ptr(&mut self) -> *mut T should vec convert
    // to array? constexpr Array(Vector<T> &vec) noexcept
    //     : item_(vec.item_)
    // {
    //     vec.item_ = nullptr;
    // }
