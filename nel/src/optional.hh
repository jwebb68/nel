...
class Optional {
  public:
    class Some {
      ...
      public:
        explicit Some(const T &other) noexcept: value_(other) {}

      public:
        // TODO: unwrap or deref operator for value_ access?
        const T &unwrap(void) const noexcept {
            return this->value_;
        }
  ...
  public:
    ...
    Optional(const Optional &other) noexcept: tag_(other.tag_) {
        switch (other.tag_) {
            case SOME:
    ...
    Optional(const Some &v) noexcept: tag_(SOME), some_(v) {}

  public:
    bool is_some(void) const noexcept {
